...
输出格式为 JSON，包含 stage、status、data 字段。"""
    ...
    async def orchestrate(
        ...
        # 持久化用户输入（Task 5 / v9.0）
        try:
            self.save_message("user", user_input, conversation_id=conversation_id or None)
        except Exception:
        ...
        # 持久化最终编排结果（Task 5 / v9.0）
        final_summary = "可进入深度辅助：文献精读 / 实验预研 / 答辩模拟"
        try:
            self.save_message(
                "assistant",
                final_summary,
                conversation_id=conversation_id or None,
                metadata={"final_stage": self.current_stage},
            )
        except Exception:
            logger.debug("持久化编排结果失败", exc_info=True)
        yield {
            "stage": "deep_assist",
            "agent_id": "orchestrator",
            "status": "completed",
