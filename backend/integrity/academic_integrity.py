...
# 数据造假：标准差异常倍数
FABRICATION_STD_MULTIPLIER = 2.0
...
def _benford_chi_square(numbers: list[float]) -> tuple[float, dict[int, float]]:
    ...
    if not numbers:
        return 0.0, {}
    first_digits = [int(str(abs(n))[0]) for n in numbers if n != 0 and abs(n) >= 1]
    if not first_digits:
        return 0.0, {}
...
class AcademicIntegrityChecker:
    ...
    def _check_figure_manipulation(self, metadata: DocumentMetadata) -> list[IntegrityIssue]:
        ...
        for idx, fig in enumerate(figures):
            ...
            # 仅对非空描述计算哈希，空描述使用唯一标识避免误报
            if desc.strip():
                hash_val = _text_hash(desc)
            else:
    ...
    def _check_citation_fabrication(
        ...
        # 引用堆叠检测：按来源（前两个词作为来源标识）分组
        source_counter: Counter[str] = Counter()
        for ref in references:
            # 提取来源标识（前两个词，便于识别同一来源的多次引用）
            words = ref.strip().split()
            source = " ".join(words[:2]) if len(words) >= 2 else ref.strip()[:30]
