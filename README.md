...
| **本地混合检索** | BM25 + FAISS 向量检索 + Qwen3-reranker 重排序，三路融合 | `backend/retrieval/`、`backend/routes/retrieval.py` |
...
| **Agent 历史持久化** | 对话记录保存到 SQLite，应用重启后自动恢复 | `backend/agents/__init__.py`（restore_all_histories） |
| **智能上下文压缩** | 超过阈值自动浓缩早期对话，保持缓存命中率 ≥95% | `backend/config.py`（COMPACT_*）、`backend/sessions/` |
| **SSE 流式输出** | 实时渲染 reasoning/content，折叠面板展示思考过程 | `backend/routes/conversations.py`、`frontend/scripts/pages/sessions.js` |
| **深度思考/联网搜索开关** | 按模型能力自动适配，前端可手动切换 | `backend/ai/ai_proxy.py`、`frontend/scripts/pages/sessions.js` |
...
| **2026.06 最新模型** | 10 个 2026.06 模型（deepseek-v4/glm-5.2/qwen3-max-2026/gpt-5 等），agent_default 路由 | `backend/config.py` |
| **全流程闭环** | 选题→开题→论文撰写→答辩准备 完整状态机 | `backend/orchestration/state_machine.py` |
| **论文撰写 Agent** | 大纲生成、逐章撰写、修订、查重降重 | `backend/agents/thesis_writer.py`、`backend/routes/thesis.py` |
| **答辩准备** | PPT 大纲、模拟问题、答辩话术、回答评估 | `backend/agents/defense_agent.py`、`backend/routes/defense.py` |
| **会话历史检索** | 多条件筛选（时间/会话/Agent/阶段/关键词） | `backend/routes/search.py`、`frontend/scripts/pages/search.js` |
| **6 套高级配色主题** | Editorial Academic / 海洋蓝 / 森林绿 / 暮光紫 / 极简白 / 赛博朋克 | `frontend/scripts/themes.js`、`frontend/styles/main.css` |
...
- **本地混合检索**：BM25（关键词检索）+ FAISS（向量检索）+ Qwen3-reranker（重排序），三路融合排序。
- **流式输出**：SSE（Server-Sent Events）实时推送 reasoning/content，前端折叠面板展示思维链。
- **多主题系统**：CSS 变量驱动的 6 套高级配色主题，运行时切换无需刷新。

### 1.4 v9.0 新特性

v9.0 在 v8.0 多 Agent 架构基础上，新增以下 14 项核心特性：

1. **本地混合检索**：BM25 + FAISS 向量检索 + Qwen3-reranker 重排序，三路融合排序，支持语义检索与精确匹配。
2. **2026.06 模型更新**：deepseek-v4、glm-5.2、qwen3-max-2026、gpt-5、gpt-5-mini、claude-sonnet-4.5 等 10 个 2026.06 最新模型，每个模型含 `capabilities` 能力开关字段。
3. **深度思考/联网搜索/流式输出开关**：按模型能力（`capabilities` 字段）自动适配，前端可手动切换，不支持能力的模型自动隐藏开关。
4. **Agent 历史持久化**：对话记录保存到 SQLite，应用重启后通过 `restore_all_histories()` 自动恢复所有 Agent 的历史消息。
5. **智能上下文压缩**：超过阈值（默认 10 轮）自动浓缩早期对话，压缩后历史进入稳定前缀，保持缓存命中率 ≥95%。
6. **SSE 流式输出与思维链展示**：实时渲染 reasoning/content，折叠面板展示思考过程，支持中断与重连。
7. **全流程闭环**：选题→开题→论文撰写→答辩准备 完整状态机，向后兼容 v8.0 五阶段。
8. **论文撰写 Agent**：大纲生成、逐章撰写、修订、查重降重，支持流式输出章节内容。
9. **答辩准备**：PPT 大纲、模拟问题、答辩话术、回答评估，覆盖答辩全流程。
10. **会话历史检索页面**：多条件筛选（时间/会话/Agent/阶段/关键词），支持跨会话检索历史消息。
11. **论文撰写页面**：章节管理、查重仪表盘、答辩准备入口，可视化论文撰写进度。
12. **6 套高级配色主题**：Editorial Academic / 海洋蓝 / 森林绿 / 暮光紫 / 极简白 / 赛博朋克，CSS 变量驱动运行时切换。
13. **谱系管理页面修复**：修复 Lucide 图标空白问题，增强 D3.js 图谱交互（拖拽/缩放/悬停高亮/类型过滤）。
14. **模型管理与预算估算修复**：修复字段映射问题，更新定价表至 2026.06 模型批次，修复预算估算报错。
...
| 文件 | 职责唯一性 |
|------|------------|
| `main.py` | 应用唯一入口。创建 FastAPI 实例（title=`ThesisMiner v9.0`，version=`9.0.0`），配置 CORS（开发环境允许所有源），通过 `asynccontextmanager` 实现的 `lifespan` 异步上下文管理器在启动时初始化数据库、从 SQLite 恢复所有 Agent 历史消息并按配置自动打开浏览器（替代已弃用的 `@app.on_event`），注册全部 12 个路由模块（含 v9.0 新增 retrieval / thesis / defense / search 路由与 cache_stats 路由），并将 `frontend/` 目录以静态文件形式挂载到根路径 `/`（仅当目录存在时挂载）。 |
...
| 编排状态机 | `backend/orchestration/state_machine.py` | v9.0 全流程状态机（选题→开题→论文撰写→答辩准备）+ 五阶段门禁 + 评分<60 回退逻辑，向后兼容 v8.0 五阶段 |
| 智能体 | `backend/agents/` | v9.0 多 Agent 架构：Orchestrator（主管理）+ Searcher（联网检索）/ Reasoner（四维创意）/ Critic（新颖性评估）/ Mentor（导师评审）/ Writer（多粒度生成）/ ThesisWriter（论文撰写）/ DefenseAgent（答辩准备），每个 Agent 继承 BaseAgent 抽象基类，独立上下文，通过 AgentRegistry 全局注册，历史持久化到 SQLite |
...
| 本地混合检索 | `backend/retrieval/` | v9.0 BM25（关键词）+ FAISS（向量）+ Qwen3-reranker（重排序）三路融合，支持语义检索与精确匹配 |
| 缓存策略 | `backend/ai/prompt_cache.py`、`backend/ai/cache_monitor.py` | 三段式 Prompt（不可变前缀 + 动态尾部），DeepSeek 缓存命中率 ≥95%，cache_hit_rate 写入 budget_ledger |
| 智能上下文压缩 | `backend/config.py`（COMPACT_*） | v9.0 超过阈值自动浓缩早期对话，压缩后历史进入稳定前缀，保持缓存命中率 ≥95% |
| 流式输出 | `backend/routes/conversations.py`（SSE） | v9.0 SSE 实时推送 reasoning/content，按模型能力自动适配深度思考/联网搜索开关 |
...
| 样式 | Tailwind CSS（CDN） + `styles/main.css` | CDN 引入并通过 `tailwind.config` 扩展自定义设计令牌，映射到 CSS 变量支持多主题 |
| 图标 | Lucide Icons（CDN） | `https://unpkg.com/lucide@0.460.0`（pinned 版本） |
| 字体 | Google Fonts | Fraunces（display）、DM Sans（body）、Noto Serif SC / Noto Sans SC（中文）、JetBrains Mono（mono） |
| 主题 | 6 套高级配色主题 | v9.0 Editorial Academic / 海洋蓝 / 森林绿 / 暮光紫 / 极简白 / 赛博朋克，CSS 变量驱动，`themes.js` 运行时切换 |
| 图谱 | D3.js v7（CDN） | 力导向布局，节点拖拽/缩放/悬停高亮/类型过滤 |
...
遵循 RESTful 风格，统一前缀 `/api`，返回 JSON。v9.0 系统共提供 **80+** 个 API 端点，分布于 12 个路由模块（含 v9.0 新增 retrieval / thesis / defense / search 路由）。
...
| GET | `/api/status` | 返回服务健康状态（status、version=9.0.0、rag_enabled、ai_configured） |
| GET | `/api/models` | 获取多模型注册表（v9.0 含 10 个 2026.06 模型，含 agent_default / release_year / capabilities 字段） |
| POST | `/api/models` | 新增模型到注册表（接收模型配置 JSON） |
| PUT | `/api/models/{model_id}` | 更新指定模型配置 |
| DELETE | `/api/models/{model_id}` | 删除模型（若被步骤路由使用则拒绝） |
| GET | `/api/step-models` | 获取步骤路由配置（v9.0 返回 orchestrator/reasoner/mentor/inspire/report/search/thesis_writer 映射） |
| PUT | `/api/step-models` | 更新步骤路由配置（校验模型存在性） |
| PUT | `/api/currency` | 切换计价货币（仅支持 CNY/USD） |
| GET | `/api/agents` | 返回 Agent 注册表元数据（id/name/description/default_model/capabilities） |
| GET | `/api/cache-stats` | 返回最近 100 次 DeepSeek 调用的平均缓存命中率 |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/retrieval/search` | 混合检索（BM25 + FAISS 向量 + Qwen3-reranker 重排序），返回融合排序结果 |
| POST | `/api/retrieval/index` | 索引文档（支持会话消息、论题、知识卡片等多源数据） |
| GET | `/api/retrieval/status` | 查询检索引擎状态（已索引文档数、向量维度、reranker 是否就绪） |
| POST | `/api/retrieval/rebuild` | 重建索引（全量重新索引） |

### 5.11 论文撰写（v9.0 新增）

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/thesis/{session_id}` | 获取会话的论文撰写状态与大纲 |
| POST | `/api/thesis/{session_id}/outline` | 生成论文大纲（基于论题与开题报告） |
| POST | `/api/thesis/{session_id}/chapter` | 逐章撰写（接收 chapter_id，流式返回内容） |
| POST | `/api/thesis/{session_id}/revise` | 修订章节（接收 chapter_id 与修订意见） |
| POST | `/api/thesis/{session_id}/plagiarism-check` | 查重检测（返回重复率与高风险段落） |
| POST | `/api/thesis/{session_id}/reduce-plagiarism` | 查重降重（对高风险段落改写） |

### 5.12 答辩准备（v9.0 新增）

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/defense/{session_id}` | 获取会话的答辩准备状态 |
| POST | `/api/defense/{session_id}/ppt-outline` | 生成 PPT 大纲（基于论文内容） |
| POST | `/api/defense/{session_id}/questions` | 生成模拟答辩问题（按章节分布） |
| POST | `/api/defense/{session_id}/script` | 生成答辩话术（开场/正文/结尾） |
| POST | `/api/defense/{session_id}/evaluate` | 评估用户回答（接收问题与回答，返回评分与建议） |

### 5.13 会话历史检索（v9.0 新增）

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/search/messages` | 多条件检索消息（时间范围/会话/Agent/阶段/关键词） |
| GET | `/api/search/sessions?keyword=` | 按关键词检索会话 |
| GET | `/api/search/filters` | 获取可用筛选条件（Agent 列表/阶段列表/时间范围） |
...
| `AI_MODEL` | `deepseek-v4` | 默认模型名称（v9.0 默认切换至 2026.06 最新 DeepSeek V4） |
...
| `COMPACT_ENABLED` | `true` | v9.0 是否启用智能上下文压缩 |
| `COMPACT_THRESHOLD` | `10` | v9.0 触发压缩的轮数 |
| `COMPACT_CHARS` | `100` | v9.0 每条压缩消息的目标字符数 |
| `COMPACT_KEEP_RECENT` | `3` | v9.0 保留为原始的最近轮数 |
...
| `APP_VERSION` | v9.0.0（应用版本号常量） |
| `APP_TITLE` | ThesisMiner v9.0（应用标题常量） |
| `DEFAULT_MODELS` | v9.0 多模型注册表默认值（10 个 2026.06 模型，含定价/能力开关/上下文长度/agent_default/release_year/capabilities） |
| `DEFAULT_STEP_MODELS` | v9.0 步骤路由默认值（orchestrator/reasoner/mentor/inspire/report/search/thesis_writer → 各 Agent 角色最优模型） |
| `COMPACT_THRESHOLD` / `COMPACT_CHARS` / `COMPACT_KEEP_RECENT` / `COMPACT_ENABLED` | v9.0 智能上下文压缩配置 |

> v9.0 模型路由优先级：显式 `model` 参数 > `get_step_model(purpose)`（按步骤路由，含 orchestrator / thesis_writer）> `settings.ai_model`（默认模型）。定价单位为"元/百万 token"，从 `models` 注册表读取，回退到 `MODEL_PRICING_LEGACY_USD`。v9.0 内置 10 个 2026.06 模型：deepseek-v4 / deepseek-r3 / glm-5.2 / glm-5.2-flash / gpt-5 / gpt-5-mini / claude-opus-5 / qwen3-max-2026 / gemini-3-pro / doubao-2.0-pro，每个模型含 `capabilities`（deep_thinking / web_search / streaming）字段，按模型能力自动适配深度思考/联网搜索/流式输出开关。
...
- **新增模型定价**：v9.0 起通过多模型注册表管理，调用 `POST /api/models` 新增模型并配置 `pricing`（元/百万 token）与 `agent_default` / `release_year` / `capabilities` 字段；或直接编辑 `data/config.json` 的 `models` 列表。未登记的模型回退到 `MODEL_PRICING_LEGACY_USD` 定价。
- **调整生成模式 token 估算**：修改 `_MODE_TOKEN_ESTIMATE`（`quick`：prompt 2000 / completion 1000；`deep`：prompt 5000 / completion 3000）。
- **按步骤路由模型**：v9.0 `step_models` 配置支持 orchestrator/reasoner/mentor/inspire/report/search/thesis_writer 七个 Agent 角色，调用 `PUT /api/step-models` 为各 Agent 指定不同模型。
- **切换计价货币**：调用 `PUT /api/currency` 在 CNY/USD 间切换。
- **DeepSeek 缓存优化**：三段式 Prompt 自动固化前缀，DeepSeek 调用缓存命中率 ≥95%，无需手动干预；通过 `GET /api/cache-stats` 查看实时命中率。
- **智能上下文压缩**：v9.0 通过 `COMPACT_*` 环境变量调整压缩阈值，压缩后历史进入稳定前缀保持缓存命中率。
- **深度思考/联网搜索/流式输出开关**：v9.0 按模型 `capabilities` 字段自动适配，前端可手动切换。

> v9.0 内置模型注册表支持 10 个 2026.06 模型：`deepseek-v4`、`deepseek-r3`、`glm-5.2`、`glm-5.2-flash`、`gpt-5`、`gpt-5-mini`、`claude-opus-5`、`qwen3-max-2026`、`gemini-3-pro`、`doubao-2.0-pro`，定价单位为"元/百万 token"，每个模型含 `agent_default`（适合的 Agent 角色）、`release_year`（发布年份）与 `capabilities`（deep_thinking / web_search / streaming 能力开关）字段。
...
测试覆盖范围（v9.0 共 450+ 用例全部通过）：
- 配置与状态：`GET /api/status`（version=9.0.0）、`GET /api/config`、`POST /api/config`
...
- 多模型注册表：models 列表（10 个 2026.06 模型）、step_models（含 orchestrator / thesis_writer）、currency、辅助函数
- 模型管理 API CRUD：增删改查模型
- 步骤路由 API：GET/PUT 步骤路由
- 货币切换 API：CNY/USD 切换与非法值拒绝
- **多 Agent 架构**：`tests/test_models_v8.py`（旧模型已移除、新模型已加入、agent_default 字段存在）、`tests/test_orchestrator.py`（五阶段顺序调用、门禁回退、子 Agent 上下文隔离）
- **多对话管理**：`tests/test_conversations.py`（多对话并存、上下文隔离、消息按 conversation_id 过滤）
- **DeepSeek 缓存**：`tests/test_cache_hit.py`（连续 10 次 DeepSeek 调用 prefix 字节级一致、cache_hit_rate ≥ 0.95）
- **五阶段约束**：`tests/test_constraints_v8.py`（五阶段门禁、新颖性评分、style_normalizer 输出无模板词）、`tests/test_state_machine_v8.py`（状态转移、回退逻辑）
- **引用解析**：`tests/test_citation_parser.py`（URL/Markdown/编号引用解析）
- **v9.0 本地混合检索**：BM25 + FAISS + reranker 三路融合、索引构建与查询
- **v9.0 论文撰写**：大纲生成、逐章撰写、查重降重
- **v9.0 答辩准备**：PPT 大纲、模拟问题、答辩话术、回答评估
- **v9.0 会话历史检索**：多条件筛选、关键词检索
- **v9.0 Agent 历史持久化**：重启后历史恢复
- **v9.0 智能上下文压缩**：阈值触发、缓存命中率保持
- **v9.0 SSE 流式输出**：reasoning/content 实时推送
- **v9.0 多主题系统**：6 套主题切换、CSS 变量驱动
- **扩展单元测试**：`tests/unit/` 与 `tests/integration/` 共 20+ 测试文件，覆盖 analytics / ml / export / knowledge / validation / routing / integrity / optimization / nlp / monitoring / planning / reasoning 等模块
...
- **D3.js** 力导向图谱：<https://d3js.org/>
- **FAISS** 向量检索：<https://faiss.ai/>
...
> 文档版本 V9.0（多 Agent 架构版 + 全流程闭环）· 最后更新 2026-06-20 · 对应应用版本 ThesisMiner v9.0.0
