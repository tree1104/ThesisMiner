    ...
    def test_each_model_has_agent_default(self):
        ...
        for model in DEFAULT_MODELS:
            ...
            assert model["agent_default"] in (
                "reasoner", "mentor", "inspire", "report", "search", "orchestrator"
            )
...
class TestNewModels2026:
    ...
    def test_doubao_1_5_pro_present(self):
        """doubao-1.5-pro 应存在。"""
        ids = _get_model_ids()
        assert "doubao-1.5-pro" in ids

    def test_new_models_have_2026_year(self):
        """2026 新模型的 release_year 应为 2026。"""
        new_ids = [
            "claude-sonnet-4.5", "claude-opus-4.5", "deepseek-v3.2",
            "deepseek-r2", "qwen3-max", "gemini-2.5-pro", "glm-4.6", "doubao-1.5-pro"
        ]
        for mid in new_ids:
    ...
    def test_retained_models_have_2025_year(self):
        """保留的 GPT-4.1 系列 release_year 应为 2025。"""
        for mid in ["gpt-4.1-mini", "gpt-4.1"]:
            model = get_model_config(mid)
            assert model is not None
            assert model["release_year"] == 2025
...
class TestDefaultStepModels:
    ...
    def test_orchestrator_routes_to_claude_sonnet(self):
        """orchestrator 应路由到 claude-sonnet-4.5。"""
        assert DEFAULT_STEP_MODELS["orchestrator"] == "claude-sonnet-4.5"

    def test_reasoner_routes_to_deepseek_r2(self):
        """reasoner 应路由到 deepseek-r2。"""
        assert DEFAULT_STEP_MODELS["reasoner"] == "deepseek-r2"

    def test_mentor_routes_to_gpt_4_1(self):
        """mentor 应路由到 gpt-4.1。"""
        assert DEFAULT_STEP_MODELS["mentor"] == "gpt-4.1"

    def test_inspire_routes_to_qwen3_max(self):
        """inspire 应路由到 qwen3-max。"""
        assert DEFAULT_STEP_MODELS["inspire"] == "qwen3-max"

    def test_report_routes_to_claude_opus(self):
        """report 应路由到 claude-opus-4.5。"""
        assert DEFAULT_STEP_MODELS["report"] == "claude-opus-4.5"

    def test_search_routes_to_deepseek_v3_2(self):
        """search 应路由到 deepseek-v3.2。"""
        assert DEFAULT_STEP_MODELS["search"] == "deepseek-v3.2"
...
class TestGetModelConfig:
    ...
    def test_get_existing_model(self):
        """获取存在的模型应返回配置字典。"""
        result = get_model_config("gpt-4.1")
        assert result is not None
        assert result["id"] == "gpt-4.1"
    ...
    def test_get_model_returns_complete_config(self):
        """返回的配置应包含所有字段。"""
        result = get_model_config("deepseek-r2")
        assert result is not None
        assert "id" in result
...
class TestSettings:
    ...
    def test_settings_default_ai_model_is_deepseek_v3_2(self):
        """默认 ai_model 应为 deepseek-v3.2（当无环境变量覆盖时）。"""
        # 使用 mock 避免环境变量/config.json 覆盖
        with patch.dict(os.environ, {"AI_MODEL": "deepseek-v3.2"}, clear=False):
            _reset_settings_singleton()
            # 临时移除 config.json 影响
            with patch("backend.config.Path.exists", return_value=False):
                settings = Settings()
                assert settings.ai_model == "deepseek-v3.2"
...
class TestAgentDefaultMapping:
    ...
    def test_deepseek_r2_is_reasoner(self):
        """deepseek-r2 的 agent_default 应为 reasoner。"""
        model = get_model_config("deepseek-r2")
        assert model["agent_default"] == "reasoner"

    def test_gpt_4_1_is_mentor(self):
        """gpt-4.1 的 agent_default 应为 mentor。"""
        model = get_model_config("gpt-4.1")
        assert model["agent_default"] == "mentor"

    def test_claude_sonnet_is_orchestrator(self):
        """claude-sonnet-4.5 的 agent_default 应为 orchestrator。"""
        model = get_model_config("claude-sonnet-4.5")
        assert model["agent_default"] == "orchestrator"

    def test_claude_opus_is_report(self):
        """claude-opus-4.5 的 agent_default 应为 report。"""
        model = get_model_config("claude-opus-4.5")
        assert model["agent_default"] == "report"

    def test_qwen3_max_is_inspire(self):
        """qwen3-max 的 agent_default 应为 inspire。"""
        model = get_model_config("qwen3-max")
        assert model["agent_default"] == "inspire"

    def test_deepseek_v3_2_is_search(self):
        """deepseek-v3.2 的 agent_default 应为 search。"""
        model = get_model_config("deepseek-v3.2")
...
class TestModelCapabilities:
    ...
    def test_deepseek_r2_supports_thinking(self):
        """deepseek-r2 应支持 thinking。"""
        model = get_model_config("deepseek-r2")
        assert model["supports_thinking"] is True

    def test_claude_sonnet_supports_web_search(self):
        """claude-sonnet-4.5 应支持 web_search。"""
        model = get_model_config("claude-sonnet-4.5")
