...
- v9.0 能力开关：get_model_capabilities / deep_thinking / web_search / streaming
- v9.0 GET /api/models 返回 capabilities 字段
"""
import asyncio
import json
import os
...
def _make_model_config(
    ...
    """构造模拟的模型配置字典（v9.0 测试辅助）。

    默认构造一个支持全部能力的 DeepSeek V4 配置。
    """
...
# v9.0 测试用模型配置常量（与 DEFAULT_MODELS 保持一致，避免被 data/config.json 覆盖）
_MOCK_DEEPSEEK_V4 = _make_model_config(
    "deepseek-v4", "deepseek",
    {"deep_thinking": True, "web_search": True, "streaming": True},
)
_MOCK_DEEPSEEK_R3 = _make_model_config(
    "deepseek-r3", "deepseek",
    {"deep_thinking": True, "web_search": False, "streaming": True},
)
_MOCK_GPT5 = _make_model_config(
    "gpt-5", "openai",
    {"deep_thinking": True, "web_search": True, "streaming": True},
)
_MOCK_CLAUDE_OPUS_5 = _make_model_config(
    "claude-opus-5", "anthropic",
    {"deep_thinking": True, "web_search": False, "streaming": True},
)
_MOCK_GLM_52 = _make_model_config(
    "glm-5.2", "zhipu",
    {"deep_thinking": True, "web_search": True, "streaming": True},
)
_MOCK_DOUBAO = _make_model_config(
    "doubao-2.0-pro", "bytecode",
    {"deep_thinking": False, "web_search": False, "streaming": True},
)

# 模型 ID -> 模拟配置 的映射表
_MOCK_MODELS = {
    "deepseek-v4": _MOCK_DEEPSEEK_V4,
    "deepseek-r3": _MOCK_DEEPSEEK_R3,
    "gpt-5": _MOCK_GPT5,
    "claude-opus-5": _MOCK_CLAUDE_OPUS_5,
    "glm-5.2": _MOCK_GLM_52,
    "doubao-2.0-pro": _MOCK_DOUBAO,
}


def _mock_get_model_config(model_id: str):
    """模拟 get_model_config，从 _MOCK_MODELS 表中查找。"""
    return _MOCK_MODELS.get(model_id)


class TestGetModelCapabilities:
    """get_model_capabilities 模型能力查询测试（v9.0）"""

    def test_returns_dict_with_three_keys(self):
        """应返回包含三个布尔键的字典"""
        with patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            caps = get_model_capabilities("deepseek-v4")
        assert isinstance(caps, dict)
        assert set(caps.keys()) == {"deep_thinking", "web_search", "streaming"}
        for v in caps.values():
            assert isinstance(v, bool)

    def test_known_model_deepseek_v4(self):
        """deepseek-v4 应支持全部三项能力"""
        with patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            caps = get_model_capabilities("deepseek-v4")
        assert caps["deep_thinking"] is True
        assert caps["web_search"] is True
        assert caps["streaming"] is True

    def test_known_model_deepseek_r3(self):
        """deepseek-r3 不支持 web_search"""
        with patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            caps = get_model_capabilities("deepseek-r3")
        assert caps["deep_thinking"] is True
        assert caps["web_search"] is False
        assert caps["streaming"] is True

    def test_known_model_gpt5(self):
        """gpt-5 应支持全部三项能力"""
        with patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
    ...
    def test_known_model_claude_opus_5(self):
        """claude-opus-5 不支持 web_search"""
        with patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            caps = get_model_capabilities("claude-opus-5")
        assert caps["deep_thinking"] is True
        assert caps["web_search"] is False
        assert caps["streaming"] is True

    def test_known_model_doubao_no_thinking(self):
        """doubao-2.0-pro 不支持 deep_thinking 与 web_search"""
        with patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            caps = get_model_capabilities("doubao-2.0-pro")
        assert caps["deep_thinking"] is False
        assert caps["web_search"] is False
        assert caps["streaming"] is True

    def test_unknown_model_returns_all_false(self):
        """未注册模型应返回全 False（streaming 也为 False）"""
        with patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            caps = get_model_capabilities("non-existent-model-xyz")
        assert caps == {
            "deep_thinking": False,
            "web_search": False,
            "streaming": False,
    ...
    def test_falls_back_to_supports_fields(self):
        """缺少 capabilities 字段时应从 supports_* 字段回退"""
        legacy_config = {
            "id": "legacy",
            "provider": "deepseek",
        ...
        with patch.object(ai_proxy, "get_model_config", return_value=legacy_config):
            caps = get_model_capabilities("legacy")
        assert caps["deep_thinking"] is True
        assert caps["web_search"] is False
        assert caps["streaming"] is True
...
class TestBuildCapabilityKwargs:
    """_build_capability_kwargs 能力参数构建测试（v9.0）"""

    def test_deep_thinking_deepseek(self):
        """DeepSeek 模型 deep_thinking 应注入 extra_body.reasoning"""
        with patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            kwargs = _build_capability_kwargs(
                "deepseek-v4", _MOCK_DEEPSEEK_V4,
                deep_thinking=True, web_search=False,
            )
        assert kwargs.get("extra_body") == {"reasoning": True}

    def test_deep_thinking_openai(self):
        """OpenAI 模型 deep_thinking 应注入 reasoning_effort"""
        with patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            kwargs = _build_capability_kwargs(
                "gpt-5", _MOCK_GPT5,
                deep_thinking=True, web_search=False,
    ...
    def test_deep_thinking_anthropic(self):
        """Anthropic 模型 deep_thinking 应注入 thinking 字段"""
        with patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            kwargs = _build_capability_kwargs(
                "claude-opus-5", _MOCK_CLAUDE_OPUS_5,
                deep_thinking=True, web_search=False,
    ...
    def test_deep_thinking_other_provider(self):
        """其他厂商 deep_thinking 应通过 extra_body.enable_thinking 透传"""
        with patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            kwargs = _build_capability_kwargs(
                "glm-5.2", _MOCK_GLM_52,
                deep_thinking=True, web_search=False,
    ...
    def test_deep_thinking_unsupported_model(self):
        """不支持思考的模型应不注入任何参数"""
        with patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            kwargs = _build_capability_kwargs(
                "doubao-2.0-pro", _MOCK_DOUBAO,
    ...
    def test_web_search_openai(self):
        """OpenAI 模型 web_search 应注入 tools 与 enable_search"""
        with patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            kwargs = _build_capability_kwargs(
                "gpt-5", _MOCK_GPT5,
                deep_thinking=False, web_search=True,
            )
        assert kwargs.get("tools") == [{"type": "web_search"}]
        assert kwargs.get("extra_body", {}).get("enable_search") is True

    def test_web_search_non_openai(self):
        """非 OpenAI 模型 web_search 应仅注入 enable_search"""
        with patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            kwargs = _build_capability_kwargs(
                "deepseek-v4", _MOCK_DEEPSEEK_V4,
                deep_thinking=False, web_search=True,
    ...
    def test_web_search_unsupported_model(self):
        """不支持联网的模型应不注入任何参数"""
        with patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            kwargs = _build_capability_kwargs(
                "deepseek-r3", _MOCK_DEEPSEEK_R3,
    ...
    def test_both_deep_thinking_and_web_search(self):
        """同时启用两项能力应合并 extra_body"""
        with patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            kwargs = _build_capability_kwargs(
                "deepseek-v4", _MOCK_DEEPSEEK_V4,
                deep_thinking=True, web_search=True,
            )
        # DeepSeek：reasoning + enable_search 应合并到同一 extra_body
...
class TestMergeExtraBody:
    ...
    def test_merge_empty_dict_noop(self):
        """合并空字典应为无操作"""
        kwargs = {"extra_body": {"reasoning": True}}
        _merge_extra_body(kwargs, {})
...
class TestCallLlmCapabilitySwitches:
    """call_llm 能力开关参数测试（v9.0）"""

    @pytest.mark.asyncio
    async def test_deep_thinking_deepseek_injects_reasoning(self):
        """deep_thinking=True + DeepSeek 模型应注入 extra_body.reasoning"""
        mock_response = _make_mock_response()
        with patch.object(ai_proxy, "get_client") as mock_get, patch(
            "backend.ai.ai_proxy.check_api_configured", return_value=True
        ), patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            mock_client = AsyncMock()
            ...
            mock_get.return_value = mock_client
            await call_llm(
                "sys", "user", model="deepseek-v4", deep_thinking=True
            )
        call_kwargs = mock_client.chat.completions.create.call_args.kwargs
        assert call_kwargs.get("extra_body", {}).get("reasoning") is True

    @pytest.mark.asyncio
    async def test_deep_thinking_openai_injects_reasoning_effort(self):
        """deep_thinking=True + GPT-5 应注入 reasoning_effort=high"""
        mock_response = _make_mock_response()
        with patch.object(ai_proxy, "get_client") as mock_get, patch(
            "backend.ai.ai_proxy.check_api_configured", return_value=True
        ), patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            ...
            mock_get.return_value = mock_client
            await call_llm("sys", "user", model="gpt-5", deep_thinking=True)
        call_kwargs = mock_client.chat.completions.create.call_args.kwargs
        assert call_kwargs.get("reasoning_effort") == "high"

    @pytest.mark.asyncio
    async def test_deep_thinking_anthropic_injects_thinking(self):
        """deep_thinking=True + Claude 应注入 thinking 字段"""
        mock_response = _make_mock_response()
        with patch.object(ai_proxy, "get_client") as mock_get, patch(
            "backend.ai.ai_proxy.check_api_configured", return_value=True
        ), patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            ...
            mock_get.return_value = mock_client
            await call_llm(
                "sys", "user", model="claude-opus-5", deep_thinking=True
            )
        call_kwargs = mock_client.chat.completions.create.call_args.kwargs
    ...
    @pytest.mark.asyncio
    async def test_deep_thinking_unsupported_model_ignored(self):
        """deep_thinking=True + 不支持思考的模型应不注入任何参数"""
        mock_response = _make_mock_response()
        with patch.object(ai_proxy, "get_client") as mock_get, patch(
            "backend.ai.ai_proxy.check_api_configured", return_value=True
        ), patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            ...
            mock_get.return_value = mock_client
            await call_llm(
                "sys", "user", model="doubao-2.0-pro", deep_thinking=True
            )
        call_kwargs = mock_client.chat.completions.create.call_args.kwargs
    ...
    @pytest.mark.asyncio
    async def test_web_search_injects_enable_search(self):
        """web_search=True + 支持联网的模型应注入 enable_search"""
        mock_response = _make_mock_response()
        with patch.object(ai_proxy, "get_client") as mock_get, patch(
            "backend.ai.ai_proxy.check_api_configured", return_value=True
        ), patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            ...
            mock_get.return_value = mock_client
            await call_llm(
                "sys", "user", model="deepseek-v4", web_search=True
            )
        call_kwargs = mock_client.chat.completions.create.call_args.kwargs
        assert call_kwargs.get("extra_body", {}).get("enable_search") is True

    @pytest.mark.asyncio
    async def test_web_search_openai_injects_tools(self):
        """web_search=True + GPT-5 应注入 tools=[{'type': 'web_search'}]"""
        mock_response = _make_mock_response()
        with patch.object(ai_proxy, "get_client") as mock_get, patch(
            "backend.ai.ai_proxy.check_api_configured", return_value=True
        ), patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            ...
            mock_get.return_value = mock_client
            await call_llm("sys", "user", model="gpt-5", web_search=True)
        call_kwargs = mock_client.chat.completions.create.call_args.kwargs
        assert call_kwargs.get("tools") == [{"type": "web_search"}]

    @pytest.mark.asyncio
    async def test_web_search_unsupported_model_ignored(self):
        """web_search=True + 不支持联网的模型应不注入任何参数"""
        mock_response = _make_mock_response()
        with patch.object(ai_proxy, "get_client") as mock_get, patch(
            "backend.ai.ai_proxy.check_api_configured", return_value=True
        ), patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            ...
            mock_get.return_value = mock_client
            await call_llm(
                "sys", "user", model="deepseek-r3", web_search=True
            )
        call_kwargs = mock_client.chat.completions.create.call_args.kwargs
        assert "tools" not in call_kwargs
        extra_body = call_kwargs.get("extra_body")
        assert extra_body is None or "enable_search" not in extra_body

    @pytest.mark.asyncio
    async def test_streaming_delegates_to_call_llm_stream(self):
        """streaming=True 应委托给 call_llm_stream 返回异步生成器"""
        async def mock_stream():
            yield {"type": "content", "content": "流式片段"}

        with patch.object(ai_proxy, "call_llm_stream") as mock_stream_fn, patch(
            "backend.ai.ai_proxy.check_api_configured", return_value=True
        ):
            mock_stream_fn.return_value = mock_stream()
            result = await call_llm(
                "sys", "user", model="deepseek-v4", streaming=True
            )
        # 应返回异步生成器
        ...
        async for chunk in result:
            chunks.append(chunk)
        assert len(chunks) == 1
        assert chunks[0]["content"] == "流式片段"
        # 验证 call_llm_stream 被调用且 deep_thinking/web_search 透传
        mock_stream_fn.assert_called_once()
        call_kwargs = mock_stream_fn.call_args.kwargs
        assert call_kwargs.get("deep_thinking") is False
        assert call_kwargs.get("web_search") is False

    @pytest.mark.asyncio
    async def test_streaming_with_deep_thinking_passes_through(self):
        """streaming=True + deep_thinking=True 应透传给 call_llm_stream"""
        async def mock_stream():
            return
            yield  # 使其成为 async generator
        ...
        with patch.object(ai_proxy, "call_llm_stream") as mock_stream_fn, patch(
            ...
            await call_llm(
                "sys", "user", model="deepseek-v4",
                streaming=True, deep_thinking=True, web_search=True,
            )
        call_kwargs = mock_stream_fn.call_args.kwargs
        assert call_kwargs.get("deep_thinking") is True
        assert call_kwargs.get("web_search") is True

    @pytest.mark.asyncio
    async def test_capability_switches_default_false(self):
        """默认不启用任何能力开关时应正常调用，不注入能力参数"""
        mock_response = _make_mock_response()
        with patch.object(ai_proxy, "get_client") as mock_get, patch(
            "backend.ai.ai_proxy.check_api_configured", return_value=True
        ), patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            ...
            mock_get.return_value = mock_client
            await call_llm("sys", "user", model="deepseek-v4")
        call_kwargs = mock_client.chat.completions.create.call_args.kwargs
        assert "reasoning_effort" not in call_kwargs
        assert "thinking" not in call_kwargs
        assert "tools" not in call_kwargs
        # extra_body 可能为 None 或不含能力相关键
        extra_body = call_kwargs.get("extra_body")
        if extra_body:
    ...
    async def test_backward_compatible_without_new_params(self):
        """不传新参数时应与旧行为完全一致（返回字典）"""
        mock_response = _make_mock_response(content="兼容回复")
        with patch.object(ai_proxy, "get_client") as mock_get, patch(
            "backend.ai.ai_proxy.check_api_configured", return_value=True
        ), patch.object(ai_proxy, "get_model_config", side_effect=_mock_get_model_config):
            mock_client = AsyncMock()
            ...
            mock_get.return_value = mock_client
            result = await call_llm("sys", "user", model="deepseek-v4")
        assert isinstance(result, dict)
        assert result["content"] == "兼容回复"


class TestCallLlmStreamCapabilitySwitches:
    """call_llm_stream 能力开关参数测试（v9.0）"""

    @pytest.mark.asyncio
    async def test_stream_deep_thinking_injects_reasoning(self):
        """流式调用 deep_thinking=True 应注入 reasoning 参数"""
        async def mock_stream():
            yield _make_mock_chunk(content="思考后回复")
    ...
    @pytest.mark.asyncio
    async def test_stream_web_search_injects_enable_search(self):
        """流式调用 web_search=True 应注入 enable_search"""
        async def mock_stream():
            yield _make_mock_chunk(content="联网搜索回复")
    ...
    @pytest.mark.asyncio
    async def test_stream_always_uses_stream_true(self):
        """流式调用应始终 stream=True"""
        async def mock_stream():
            yield _make_mock_chunk(content="流式")
        ...
        with patch.object(ai_proxy, "get_client") as mock_get, patch(
            ...
            mock_get.return_value = mock_client
            async for _ in call_llm_stream("sys", "user", model="deepseek-v4"):
                pass
        call_kwargs = mock_client.chat.completions.create.call_args.kwargs
        assert call_kwargs["stream"] is True


class TestCallLlmJsonCapabilitySwitches:
    """call_llm_json 能力开关透传测试（v9.0）"""

    @pytest.mark.asyncio
    async def test_json_passes_deep_thinking(self):
        """call_llm_json 应透传 deep_thinking 给 call_llm"""
        json_content = json.dumps({"key": "value"}, ensure_ascii=False)
        mock_response = _make_mock_response(content=json_content)
        with patch.object(ai_proxy, "get_client") as mock_get, patch(
            "backend.ai.ai_proxy.check_api_configured", return_value=True
    ...
    @pytest.mark.asyncio
    async def test_json_passes_web_search(self):
        """call_llm_json 应透传 web_search 给 call_llm"""
        json_content = json.dumps({"key": "value"}, ensure_ascii=False)
        mock_response = _make_mock_response(content=json_content)
        with patch.object(ai_proxy, "get_client") as mock_get, patch(
            "backend.ai.ai_proxy.check_api_configured", return_value=True
...
class TestModelsEndpointCapabilities:
    """GET /api/models 返回 capabilities 字段测试（v9.0）"""

    def test_models_endpoint_returns_capabilities(self):
        """GET /api/models 每个模型应包含 capabilities 字段"""
        from backend.routes.config import _normalize_model_for_response
        # 使用模拟模型列表，避免被 data/config.json 影响
        mock_models = list(_MOCK_MODELS.values())
        for model in mock_models:
            normalized = _normalize_model_for_response(model)
            assert "capabilities" in normalized
            caps = normalized["capabilities"]
            assert isinstance(caps, dict)
            assert set(caps.keys()) == {
                "deep_thinking", "web_search", "streaming"
            }
            for v in caps.values():
                assert isinstance(v, bool)

    def test_models_endpoint_returns_required_fields(self):
        """GET /api/models 每个模型应包含所有必需字段"""
        from backend.routes.config import _normalize_model_for_response
        mock_models = list(_MOCK_MODELS.values())
        required_fields = {
            "id", "name", "provider", "pricing",
            "context_length", "capabilities",
            "agent_default", "release_year",
        }
        for model in mock_models:
            normalized = _normalize_model_for_response(model)
            assert required_fields.issubset(set(normalized.keys()))

    def test_normalize_deepseek_v4_capabilities(self):
        """deepseek-v4 归一化后 capabilities 应全为 True"""
        from backend.routes.config import _normalize_model_for_response
        normalized = _normalize_model_for_response(_MOCK_DEEPSEEK_V4)
        caps = normalized["capabilities"]
        assert caps["deep_thinking"] is True
        assert caps["web_search"] is True
        assert caps["streaming"] is True

    def test_normalize_falls_back_to_supports_fields(self):
        """缺少 capabilities 字段时应从 supports_* 字段回退"""
        from backend.routes.config import _normalize_model_for_response
        legacy_model = {
            "id": "legacy-model",
        ...
        normalized = _normalize_model_for_response(legacy_model)
        caps = normalized["capabilities"]
        assert caps["deep_thinking"] is True
        assert caps["web_search"] is False
        assert caps["streaming"] is True

    def test_list_models_endpoint_integration(self):
        """list_models 端点应返回归一化后的模型列表"""
        import asyncio
        from backend.routes.config import list_models
        # mock get_settings 返回包含模拟模型的 settings
        mock_settings = MagicMock()
        mock_settings.models = list(_MOCK_MODELS.values())
        with patch("backend.routes.config.get_settings", return_value=mock_settings):
