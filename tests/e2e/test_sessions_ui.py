...
def test_sessions_js_has_streaming():
    ...
    content = _read_js("sessions.js")
    assert "startStreaming" in content, "缺少流式启动方法"
    assert "pollStream" in content, "缺少流式轮询方法"
    assert "typewriterStream" in content, "缺少打字机动画方法"
    ...
    assert "knownMessageIds" in content, "缺少已知消息集合"
    # 流式配置
    assert "STREAM_POLL_INTERVAL" in content, "缺少轮询间隔配置"
    assert "STREAM_POLL_TIMEOUT" in content, "缺少轮询超时配置"
