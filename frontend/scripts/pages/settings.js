...
(function () {
  ...
  window.Pages.settings = {
          ...
          <div class="card mb-lg">
            <div class="card__header">
              <div>
                ...
                <div class="card__subtitle" id="pricing-subtitle">每百万 Token 单价</div>
              </div>
    ...
    modelCard(m) {
      ...
      // 兼容新旧字段：优先 capabilities，回退 supports_*
      const streaming = capabilities.streaming != null ? capabilities.streaming : m.supports_streaming;
      const deepThinking = capabilities.deep_thinking != null ? capabilities.deep_thinking : m.supports_thinking;
      const webSearch = capabilities.web_search != null ? capabilities.web_search : m.supports_web_search;
      const badges = [];
      if (streaming) badges.push('<span class="badge badge--default" style="font-size:0.6rem">流式</span>');
      if (deepThinking) badges.push('<span class="badge badge--default" style="font-size:0.6rem">思考</span>');
      if (webSearch) badges.push('<span class="badge badge--default" style="font-size:0.6rem">联网</span>');
      if (m.agent_default) badges.push(`<span class="badge badge--accent" style="font-size:0.6rem">${escapeHtml(m.agent_default)}</span>`);

      const displayName = m.name || m.label || m.id;
      const contextLen = m.context_length || m.max_context || 32768;
      const inputPrice = pricing.input_cny_per_million != null ? pricing.input_cny_per_million : pricing.input;
      const outputPrice = pricing.output_cny_per_million != null ? pricing.output_cny_per_million : pricing.output;
      ...
      return `
        <div class="card mb-sm" data-model-id="${escapeHtml(m.id)}" style="padding:var(--space-md);">
          <div class="card__header" style="padding:0;">
            <div class="flex-1">
              <div class="flex items-center gap-sm">
                ...
                ${m.provider ? `<span class="text-xs text-muted">${escapeHtml(m.provider)}</span>` : ''}
              </div>
          ...
          <div class="text-xs text-muted" style="margin-top:var(--space-xs);">
            输入 ¥${inputPrice || 0}/百万 · 输出 ¥${outputPrice || 0}/百万 · 上下文 ${contextLen}
          </div>
        </div>
      `;
    },
    ...
    openModelDrawer(model) {
      ...
      const m = model || {
        ...
        capabilities: { deep_thinking: false, web_search: false, streaming: true },
      ...
      const webSearch = capabilities.web_search != null ? capabilities.web_search : m.supports_web_search;
      const contextLen = m.context_length || m.max_context || 32768;
      const displayName = m.name || m.label || '';
      ...
      showDrawer({
        ...
        bodyHtml: `
          <div class="form-group">
            ...
            <input type="text" class="form-control" id="m-id" value="${escapeHtml(m.id)}" ${isEdit ? 'disabled' : ''} placeholder="如 deepseek-v4" />
            <div class="form-hint">唯一标识，创建后不可修改。</div>
          </div>
          <div class="grid grid--2">
            <div class="form-group">
              <label class="form-label" for="m-name">显示名称</label>
              <input type="text" class="form-control" id="m-name" value="${escapeHtml(displayName)}" placeholder="如 DeepSeek V4" />
            </div>
            <div class="form-group">
              <label class="form-label" for="m-provider">提供方</label>
              <input type="text" class="form-control" id="m-provider" value="${escapeHtml(m.provider || '')}" placeholder="如 deepseek" />
            </div>
          </div>
          <div class="form-group">
          ...
          <div class="grid grid--2">
            <div class="form-group">
              ...
              <input type="number" class="form-control" id="m-input" value="${pricing.input_cny_per_million != null ? pricing.input_cny_per_million : (pricing.input || 0)}" step="0.01" min="0" />
            </div>
            <div class="form-group">
              <label class="form-label" for="m-output">输出价格（¥/百万 token）</label>
              <input type="number" class="form-control" id="m-output" value="${pricing.output_cny_per_million != null ? pricing.output_cny_per_million : (pricing.output || 0)}" step="0.01" min="0" />
            </div>
          </div>
          <div class="grid grid--2">
            <div class="form-group">
              <label class="form-label" for="m-context">上下文长度</label>
              <input type="number" class="form-control" id="m-context" value="${contextLen}" step="1" min="1" />
            </div>
            <div class="form-group">
          ...
          <div class="grid grid--2">
            <div class="form-group">
              <label class="form-label" for="m-agent">默认步骤角色</label>
              <input type="text" class="form-control" id="m-agent" value="${escapeHtml(m.agent_default || '')}" placeholder="如 reasoner" />
            </div>
            <div class="form-group">
              <label class="form-label" for="m-year">发布年份</label>
              <input type="number" class="form-control" id="m-year" value="${m.release_year || 2026}" step="1" min="2000" max="2100" />
            </div>
          </div>
          <div class="form-group" style="margin-bottom:0;">
            <div class="flex flex-col gap-sm">
              <label class="flex items-center gap-sm" style="cursor:pointer;">
                <input type="checkbox" id="m-stream" ${streaming ? 'checked' : ''} />
                <span class="text-sm">支持流式输出</span>
              </label>
              <label class="flex items-center gap-sm" style="cursor:pointer;">
                <input type="checkbox" id="m-think" ${deepThinking ? 'checked' : ''} />
                <span class="text-sm">支持深度思考</span>
              </label>
              <label class="flex items-center gap-sm" style="cursor:pointer;">
                <input type="checkbox" id="m-web" ${webSearch ? 'checked' : ''} />
    ...
    async saveModelFromDrawer(isEdit, originalModel) {
      ...
      const streaming = document.getElementById('m-stream').checked;
      const deepThinking = document.getElementById('m-think').checked;
      const webSearch = document.getElementById('m-web').checked;
      const contextLength = Number(document.getElementById('m-context').value || 32768);
      const inputPrice = Number(document.getElementById('m-input').value || 0);
      const outputPrice = Number(document.getElementById('m-output').value || 0);
      const payload = {
        id: id,
        name: nameVal,
        label: nameVal, // 旧字段兼容
        ...
        pricing: {
          input: inputPrice,
          output: outputPrice,
          input_cny_per_million: inputPrice,
          output_cny_per_million: outputPrice,
        },
        context_length: contextLength,
        max_context: contextLength, // 旧字段兼容
        capabilities: {
        ...
        supports_thinking: deepThinking,
        supports_web_search: webSearch,
        agent_default: (document.getElementById('m-agent').value || '').trim(),
        release_year: Number(document.getElementById('m-year').value || 2026),
    ...
    renderStepModels(stepModels) {
      ...
      const options = models
        .map((m) => `<option value="${escapeHtml(m.id)}">${escapeHtml(m.name || m.label || m.id)}</option>`)
        .join('');
    ...
    async loadPricing() {
      ...
      try {
        ...
        // 后端返回 { pricing: {modelId: {label, input_cny_per_million, output_cny_per_million, currency}}, currency, unit }
        const pricingMap = (resp && resp.pricing) ? resp.pricing : (resp || {});
        const currency = (resp && resp.currency) || this._currency || 'CNY';
        const prefix = currency === 'USD' ? '$' : '¥';
        // 更新定价表副标题单位
        const subtitleEl = document.getElementById('pricing-subtitle');
        if (subtitleEl && resp && resp.unit) {
          subtitleEl.textContent = resp.unit;
        }
        ...
        body.innerHTML = entries
          .map(([model, p]) => {
            const label = p.label || model;
            const inputPrice = p.input_cny_per_million != null ? p.input_cny_per_million : (p.input || 0);
            const outputPrice = p.output_cny_per_million != null ? p.output_cny_per_million : (p.output || 0);
            return `<tr>
              <td class="table__cell-primary"><span class="text-mono">${escapeHtml(label)}</span></td>
              <td class="table__cell-mono">${prefix}${Number(inputPrice).toFixed(2)}</td>
              <td class="table__cell-mono">${prefix}${Number(outputPrice).toFixed(2)}</td>
            </tr>`;
          })
          .join('');
      } catch (err) {
