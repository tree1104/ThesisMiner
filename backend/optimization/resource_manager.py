...
class ResourceAlert:
    ...
    def __init__(self, level=AlertLevel.WARNING, resource_type=ResourceType.CPU,
                 message="", value=0.0, threshold=0.0, timestamp="",
                 current_value=None, id=""):
        self.id = id or str(uuid.uuid4())
        self.level = level
        self.resource_type = resource_type
        self.message = message
        # current_value 是 value 的别名
        self.value = current_value if current_value is not None else value
...
class SlidingWindowRateLimiter(RateLimiter):
    ...
    def __init__(
        ...
        # rate 是 window_size 的别名（向后兼容）：rate 越大窗口越小
        if rate is not None and rate > 0:
            window_size = capacity / rate if capacity > 0 else 1.0
        super().__init__(capacity, capacity / window_size if window_size > 0 else capacity)
