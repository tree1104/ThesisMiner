...
def validate_discipline(discipline: str, field: str = "discipline") -> ValidationResult:
    ...
    if discipline not in VALID_DISCIPLINES:
        ...
        result.errors.append(
            InvalidEnumValueError(field, discipline, VALID_DISCIPLINES)
        )
        result.is_valid = True  # 自定义学科视为有效但给出警告
        return result
...
def validate_currency(currency: str, field: str = "currency") -> ValidationResult:
    """验证货币类型。"""
    return validate_enum(currency, VALID_CURRENCIES, field=field, case_sensitive=False)
...
def validate_mentor_info(mentor_info: str, field: str = "mentor_info") -> ValidationResult:
    ...
    """
    result = validate_string(mentor_info, field=field, max_length=5000, allow_empty=True)
    if not result:
        return result
...
def validate_pagination(
    limit: Any, offset: Any, max_limit: int = 100,
) -> ValidationResult:
    """验证分页参数。"""
    result = ValidationResult(is_valid=True)
    ...
    if result.is_valid:
        result.value = {"limit": int(limit), "offset": int(offset)}
    return result
