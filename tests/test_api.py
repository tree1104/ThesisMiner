...
def test_step_models_api():
    ...
    # PUT - update step-models（v9.0 使用 2026.06 模型 ID）
    response = client.put("/api/step-models", json={"reasoner": "gpt-5-mini"})
    assert response.status_code == 200
    assert response.json()["success"] is True

    # Verify update
    response = client.get("/api/step-models")
    assert response.json()["step_models"]["reasoner"] == "gpt-5-mini"
...
def test_estimate_cost_cny():
    ...
    # Test with a model in the registry (v9.0: deepseek-v4)
    cost_cny = estimate_cost("deepseek-v4", 1000000, 500000, currency="CNY")
    # deepseek-v4: input 4/M, output 16/M
    # 1M * 4 + 0.5M * 16 = 4 + 8 = 12
    assert abs(cost_cny - 12.0) < 0.01

    # Test USD conversion
    cost_usd = estimate_cost("deepseek-v4", 1000000, 500000, currency="USD")
    # 12 CNY / 7.2 = 1.6667 USD
    assert abs(cost_usd - 12.0 / 7.2) < 0.01
