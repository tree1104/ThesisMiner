...
class TestTokenCounter:
    ...
    def test_count_with_response(self):
        ...
        response = "这是回答内容"
        tokens = counter.count_with_response(messages, response)
        assert tokens > 0
    ...
    def test_max_context_for_model(self):
        ...
        counter = TokenCounter()
        # 测试已知模型
        ctx = counter.max_context_for_model("gpt-4")
        assert ctx > 0

    def test_max_context_unknown_model(self):
        """测试未知模型的最大上下文。"""
        counter = TokenCounter()
        ctx = counter.max_context_for_model("unknown-model-xyz")
        # 应该返回默认值或 0
        assert ctx >= 0
...
class TestContextWindow:
    ...
    def test_trim_to_fit(self):
        """测试裁剪以适应窗口。"""
        window = ContextWindow(max_tokens=100, reserved_for_response=50)
        # 添加多条消息使超出限制
        for i in range(20):
...
class TestContextCompressor:
    ...
    def test_should_compress_below_threshold(self):
        ...
        # 利用率低，不需要压缩
        result = compressor.should_compress(messages, max_tokens=4096)
    ...
    def test_should_compress_above_threshold(self):
        ...
        result = compressor.should_compress(messages, max_tokens=500)
...
class TestEdgeCases:
    ...
    def test_very_small_window(self):
        """测试极小窗口。"""
        window = ContextWindow(max_tokens=10, reserved_for_response=5)
