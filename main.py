...
from backend.agents import restore_all_histories
...
from backend.routes import retrieval as retrieval_router
from backend.routes import thesis as thesis_router
from backend.routes import defense as defense_router
...
async def lifespan(app: FastAPI):
    ...
    # Task 5 / v9.0：从数据库恢复所有 Agent 的历史消息
    try:
        restored = restore_all_histories()
        if restored:
            total = sum(restored.values())
            logger.info(
                "Agent 历史恢复完成，共 %d 条消息，分布：%s", total, restored
            )
    except Exception:
        logger.warning("Agent 历史恢复失败", exc_info=True)
...
# v9.0 Task 2.6：本地混合检索路由（路径 /api/retrieval/*）
app.include_router(retrieval_router.router)
# v9.0 Task 9：论文撰写路由（路径 /api/thesis/{session_id}/*）
app.include_router(thesis_router.router)
# v9.0 Task 10：答辩准备路由（路径 /api/defense/{session_id}/*）
app.include_router(defense_router.router)
# v9.0 Task 12：会话历史检索路由（路径 /api/search/*）
