...
v9.0: 扩展为全流程闭环（选题→开题→完成论文），
      新增 THESIS_WRITING / DEFENSE_PREP / COMPLETED 阶段，
      覆盖论文撰写与答辩准备，保持 v8.0 既有五阶段向后兼容。
"""
...
class Stage(str, Enum):
    ...
    v8.0 五阶段：INFO_CONFIRM → CREATIVITY → VALIDATION → GENERATION → DEEP_ASSIST
    v9.0 扩展：→ THESIS_WRITING → DEFENSE_PREP → COMPLETED
    """
    ...
    # v9.0 新增阶段
    THESIS_WRITING = "thesis_writing"    # 论文撰写
    DEFENSE_PREP = "defense_prep"        # 答辩准备
...
class Event(str, Enum):
    ...
    RESET = "reset"
    # v9.0 新增事件
    ENTER_THESIS_WRITING = "enter_thesis_writing"    # 进入论文撰写
    ENTER_DEFENSE_PREP = "enter_defense_prep"        # 进入答辩准备
    DEFENSE_COMPLETED = "defense_completed"          # 答辩完成
...
TRANSITIONS = {
    ...
    # v9.0 全流程扩展转移
    (Stage.DEEP_ASSIST, Event.ENTER_THESIS_WRITING): Stage.THESIS_WRITING,
    (Stage.GENERATION, Event.ENTER_THESIS_WRITING): Stage.THESIS_WRITING,  # 跳过深度辅助
    (Stage.THESIS_WRITING, Event.ENTER_DEFENSE_PREP): Stage.DEFENSE_PREP,
    (Stage.DEFENSE_PREP, Event.DEFENSE_COMPLETED): Stage.COMPLETED,
    (Stage.COMPLETED, Event.RESET): Stage.INFO_CONFIRM,
}
...
def transition(current_stage: Stage, event: Event) -> TransitionResult:
    ...
    messages = {
        ...
        # v9.0 新增转移消息
        (Stage.DEEP_ASSIST, Event.ENTER_THESIS_WRITING): "深度辅助完成，进入论文撰写阶段",
        (Stage.GENERATION, Event.ENTER_THESIS_WRITING): "跳过深度辅助，直接进入论文撰写阶段",
        (Stage.THESIS_WRITING, Event.ENTER_DEFENSE_PREP): "论文撰写完成，进入答辩准备阶段",
        (Stage.DEFENSE_PREP, Event.DEFENSE_COMPLETED): "答辩准备完成，流程结束",
        (Stage.COMPLETED, Event.RESET): "重置流程",
    }
