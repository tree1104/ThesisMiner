...
JOIN_PATTERN = re.compile(
    ...
    r"(?:\s+(?:AS\s+)?([a-zA-Z_][a-zA-Z0-9_]*))?\s+ON\s+(.+?)(?=\s+(?:INNER|LEFT|RIGHT|FULL|CROSS|JOIN|WHERE|GROUP|ORDER|LIMIT|UNION|$))",
    re.IGNORECASE,
)
...
def _extract_functions(sql: str) -> list[str]:
    ...
    for match in func_pattern.finditer(sql):
        ...
        if func not in SQL_KEYWORDS and func not in functions:
