...
| 谱系管理 | 学术谱系图（节点 / 边）的导入、查询、搜索、删除，知识卡片管理，图谱扩展（从论文摘要抽取实体关系），分页查询与批量删除 | `backend/knowledge/`、`backend/routes/lineage.py` |
| 会话管理 | 会话的创建、列表、详情、删除、状态更新，上下文压缩（保留最近 N 条历史），对话轮数统计 | `backend/sessions/`、`backend/routes/sessions.py` |
| 预算控制 | 按学位分级路由模型，透明账本记录每次 AI 调用的 token 用量与费用（含缓存命中细分），提供明细查询、汇总统计与会话级费用估算 | `backend/budgets/`、`backend/routes/budgets.py` |
| 真实文献检索 | 支持模拟/真实检索热插拔（arXiv + Semantic Scholar），5秒超时自动降级 | `backend/agents/searcher_wrapper.py`、`backend/routes/constraints.py` |
| 开题报告直出 | 基于论题数据一键生成标准 Markdown 开题报告（AI增强+模板兜底） | `backend/agents/proposal_writer.py`、`backend/routes/proposals.py` |
| 硬约束拦截 | 标题/周期强制校验，不合规返回 HTTP 422 | `backend/orchestration/hooks/hard_rule_interceptor.py` |
| Prompt缓存架构 | 三段式Prompt + DST状态压缩 + SHA-256前缀哈希 | `backend/ai/prompts.py`、`backend/sessions/` |
| 多模型注册表 | 内置 6 个模型（gpt-4.1-mini/gpt-4.1/deepseek-chat-v3/deepseek-reasoner/qwen-plus/qwen-max），支持模型 CRUD、按步骤路由（reasoner/mentor/inspire/report/search）、货币切换（CNY/USD） | `backend/config.py`、`backend/routes/config.py` |
| 自动开浏览器 | lifespan 启动钩子，启动时按配置自动打开浏览器（`auto_open_browser` 配置项） | `main.py` |
...
| 文件 | 职责唯一性 |
|------|------------|
| `main.py` | 应用唯一入口。创建 FastAPI 实例（title=`ThesisMiner v7.0`，version=`7.0.0`），配置 CORS（开发环境允许所有源），通过 `asynccontextmanager` 实现的 `lifespan` 异步上下文管理器在启动时初始化数据库并按配置自动打开浏览器（替代已弃用的 `@app.on_event`），注册全部 7 个路由模块，并将 `frontend/` 目录以静态文件形式挂载到根路径 `/`（仅当目录存在时挂载）。 |
...
| 智能体 | `backend/agents/` | Reasoner（论题生成）、Mentor（导师评审）、Searcher（模拟检索与新颖性检查）、ProposalWriter（开题报告直出） |
...
遵循 RESTful 风格，统一前缀 `/api`，返回 JSON。系统共提供 **44 个** API 端点，分布于 7 个路由模块。
...
| GET | `/api/models` | 获取多模型注册表（v7.0 新增，返回 models 列表与 count） |
| POST | `/api/models` | 新增模型到注册表（v7.0 新增，接收模型配置 JSON） |
| PUT | `/api/models/{model_id}` | 更新指定模型配置（v7.0 新增） |
| DELETE | `/api/models/{model_id}` | 删除模型（v7.0 新增，若被步骤路由使用则拒绝） |
| GET | `/api/step-models` | 获取步骤路由配置（v7.0 新增，返回 reasoner/mentor/inspire/report/search 映射） |
| PUT | `/api/step-models` | 更新步骤路由配置（v7.0 新增，校验模型存在性） |
| PUT | `/api/currency` | 切换计价货币（v7.0 新增，仅支持 CNY/USD） |
...
| GET | `/api/sessions?limit=&offset=` | 分页查询会话列表（按创建时间降序），返回 `dialog_rounds` 对话轮数（v7.0 增强） |
| GET | `/api/sessions/{session_id}` | 获取会话详情（不存在返回 404），含 `dialog_rounds` 对话轮数（v7.0 增强） |
...
| GET | `/api/budgets/summary` | 获取账本汇总统计（总调用、总 token、总费用、input_cached/input_uncached/output 三类细分、按模型 / 用途分组）（v7.0 增强） |
| GET | `/api/budgets/session/{session_id}` | 获取指定会话的费用统计（含三类 token 细分）（v7.0 增强） |
| GET | `/api/budgets/pricing` | 获取模型定价表（从模型注册表读取，元/百万 token，支持 CNY/USD 货币切换）（v7.0 增强） |
...
| `DEFAULT_MODELS` | v7.0 多模型注册表默认值（6 个模型，含定价/能力开关/上下文长度） |
| `DEFAULT_STEP_MODELS` | v7.0 步骤路由默认值（reasoner/mentor/inspire/report/search → 第一个模型） |

> v7.0 模型路由优先级：显式 `model` 参数 > `get_step_model(purpose)`（按步骤路由）> `settings.ai_model`（默认模型）。定价单位为"元/百万 token"，从 `models` 注册表读取，回退到 `MODEL_PRICING_LEGACY_USD`。
...
- **新增模型定价**：v7.0 起通过多模型注册表管理，调用 `POST /api/models` 新增模型并配置 `pricing`（元/百万 token）；或直接编辑 `data/config.json` 的 `models` 列表。未登记的模型回退到 `MODEL_PRICING_LEGACY_USD` 定价。
- **调整生成模式 token 估算**：修改 `_MODE_TOKEN_ESTIMATE`（`quick`：prompt 2000 / completion 1000；`deep`：prompt 5000 / completion 3000）。
- **按步骤路由模型**：v7.0 新增 `step_models` 配置，调用 `PUT /api/step-models` 为 reasoner/mentor/inspire/report/search 各步骤指定不同模型。
- **切换计价货币**：v7.0 新增 `currency` 配置，调用 `PUT /api/currency` 在 CNY/USD 间切换。

> v7.0 内置模型注册表支持：`gpt-4.1-mini`、`gpt-4.1`、`deepseek-chat-v3`、`deepseek-reasoner`、`qwen-plus`、`qwen-max`，定价单位为"元/百万 token"。
...
- 谱系管理：导入与查询、图谱、搜索、知识卡片增查、分页查询、批量删除端点（v7.0 新增）
- 创意引擎：创意激发、跨域联想、候选排序（验证 `mentor_project` 权重最高排首位）
- 约束校验：标题校验（合法 / 超长）、可行性校验（可行 / 超期）、文献基线校验（充足 / 不足）、学术日历查询、文献基线查询
- 会话管理：完整 CRUD（创建 / 列表 / 详情 / 状态更新 / 删除）、对话轮数统计（v7.0 新增）
- 论题生成：列表查询、未配置 API Key 时正确返回 400
- 预算控制：估算、汇总、账本、定价表、三类 token 统计、估算器人民币定价（v7.0 新增）
- 异常处理：404 处理
- 会话级联删除：删除会话后 proposals 与 budget_ledger 同步清理验证
...
- lifespan 启动：无 on_event 弃用告警（v7.0 新增）
- auto_open_browser 配置项存在（v7.0 新增）
- 多模型注册表：models 列表、step_models、currency、辅助函数（v7.0 新增）
- 模型管理 API CRUD：增删改查模型（v7.0 新增）
- 步骤路由 API：GET/PUT 步骤路由（v7.0 新增）
- 货币切换 API：CNY/USD 切换与非法值拒绝（v7.0 新增）
...
> 文档版本 V5.0（增强版）· 最后更新 2026-06-19 · 对应应用版本 ThesisMiner v7.0
