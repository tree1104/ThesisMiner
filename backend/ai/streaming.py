    ...
    def _parse_openai_chunk(self, data: dict) -> Optional[StreamChunk]:
        ...
        if not choices:
            ...
            if "usage" in data:
                ...
                self._chunk_index += 1
                return chunk
            # 兼容 StreamingResponseBuilder 产生的简单格式
            ...
            if content or reasoning:
                if reasoning and not content:
                    ...
                    # 两者都有：按 content 处理（与下方 OpenAI 分支一致）
                    event_type = StreamEventType.CONTENT_DELTA.value
                    self.stats.content_chunks += 1
                    self.stats.total_content_chars += len(content)
                chunk = StreamChunk(
...
async def format_stream_as_sse(
    stream: AsyncIterator,
    citations: Optional[list] = None,
) -> AsyncGenerator[str, None]:
    """将 call_llm_stream 的输出转换为符合 v9.0 Task 7 规范的 SSE 字符串流。

    输入流产出形如 {"type": "reasoning"/"content", "content": "..."} 的字典，
    本函数将其转换为 SSE 事件字符串：

        data: {"type": "reasoning", "content": "thinking..."}\n\n
        data: {"type": "content", "content": "response..."}\n\n
        data: {"type": "done", "content": "full response",
               "reasoning": "full reasoning", "citations": []}\n\n

    在流结束时聚合完整内容与推理，并附带 citations（默认空列表）。
    若流处理过程中抛出异常，则发射一个 error 事件后终止。

    Args:
        stream: 异步流，产出 {"type": str, "content": str} 字典。
        citations: 可选的引用列表，附加在 done 事件中。
    ...
    try:
        async for chunk in stream:
            ...
            if isinstance(chunk, dict):
                ...
                chunk_type = getattr(chunk, "type", "") or getattr(chunk, "event_type", "")
                chunk_content = getattr(chunk, "content", "") or getattr(chunk, "reasoning", "")

            if chunk_type == "reasoning" and chunk_content:
            ...
            # 其他类型（start/usage/metadata 等）静默跳过，保持前端兼容
...
def _encode_sse(payload: dict) -> str:
    """将字典编码为单条 SSE 事件字符串（data: 前缀 + 双换行结尾）。
