...
def retry(func: Optional[Callable] = None, max_retries: int = 3, delay: float = 1.0, backoff: float = 2.0, exceptions: tuple = (Exception,)) -> Callable:
    """同步重试装饰器。

    支持两种用法：
        @retry                          # 裸装饰器
        @retry(max_retries=2, delay=0)  # 带参数
    """
    import functools

    def _make_wrapper(wrapped_func: Callable) -> Callable:
