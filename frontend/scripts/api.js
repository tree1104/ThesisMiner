...
const API = {
  ...
  /**
   * 多条件检索会话消息
   * @param {object} params - { q, session_id, agent_id, stage, date_from, date_to, page, page_size }
   * @returns {Promise<object>} { total, page, page_size, results[] }
   */
  searchMessages: (params = {}) => {
    ...
    return API.request(`/search/messages?${query.toString()}`);
  },

  /** 获取可用于筛选的会话列表（仅含存在消息记录的会话） */
  getSearchSessions: () => API.request('/search/sessions'),

  /* ------------------------------------------------------------------------
     v9.0 Task 13：论文撰写
     ------------------------------------------------------------------------ */
  /** 生成论文大纲 */
  generateOutline: (sessionId, data = {}) =>
    API.request(`/thesis/${sessionId}/outline`, { method: 'POST', body: JSON.stringify(data) }),

  /** 撰写指定章节 */
  generateChapter: (sessionId, data = {}) =>
    API.request(`/thesis/${sessionId}/chapter`, { method: 'POST', body: JSON.stringify(data) }),

  /** 修订指定章节 */
  reviseChapter: (sessionId, data = {}) =>
    API.request(`/thesis/${sessionId}/revise`, { method: 'POST', body: JSON.stringify(data) }),

  /** 查重检测 */
  checkPlagiarism: (sessionId, data = {}) =>
    API.request(`/thesis/${sessionId}/plagiarism`, { method: 'POST', body: JSON.stringify(data) }),

  /** 降重改写 */
  reduceSimilarity: (sessionId, data = {}) =>
    API.request(`/thesis/${sessionId}/reduce-similarity`, { method: 'POST', body: JSON.stringify(data) }),

  /** 获取全部章节列表 */
  getChapters: (sessionId) =>
    API.request(`/thesis/${sessionId}/chapters`),

  /** 更新章节内容 */
  updateChapter: (sessionId, chapterId, data = {}) =>
    API.request(`/thesis/${sessionId}/chapters/${chapterId}`, { method: 'PUT', body: JSON.stringify(data) }),

  /* ------------------------------------------------------------------------
     v9.0 Task 13：答辩准备
     ------------------------------------------------------------------------ */
  /** 生成答辩 PPT 大纲 */
  generateDefensePpt: (sessionId, data = {}) =>
    API.request(`/defense/${sessionId}/ppt`, { method: 'POST', body: JSON.stringify(data) }),

  /** 生成答辩问题 */
  generateDefenseQuestions: (sessionId, data = {}) =>
    API.request(`/defense/${sessionId}/questions`, { method: 'POST', body: JSON.stringify(data) }),

  /** 评估答辩回答 */
  evaluateDefenseAnswer: (sessionId, data = {}) =>
    API.request(`/defense/${sessionId}/evaluate`, { method: 'POST', body: JSON.stringify(data) }),

  /** 生成答辩演讲稿 */
  generateDefenseSpeech: (sessionId, data = {}) =>
    API.request(`/defense/${sessionId}/speech`, { method: 'POST', body: JSON.stringify(data) }),
};
